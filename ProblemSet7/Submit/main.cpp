...
void applySmoothing(float normal[][200][3], float newNormal[][200][3]){
    ...
    for (int i=0; i<width; ++i)
        for (int j=0;j<height;++j){
            ...
            if (i==0 || j==0 || i == width-1 || j==height-1)
            {
                ...
                newNormal[i][j][0] = normal[i][j][0];
                newNormal[i][j][1] = normal[i][j][1];
                newNormal[i][j][2] = normal[i][j][2];
                ...
                float normal1[] = {normal[i+1][j][0], normal[i+1][j][1], normal[i+1][j][2]};
                float normal2[] = {normal[i-1][j][0], normal[i-1][j][1], normal[i-1][j][2]};
                float normal3[] = {normal[i][j+1][0], normal[i][j+1][1], normal[i][j+1][2]};
                float normal4[] = {normal[i][j-1][0], normal[i][j-1][1], normal[i][j-1][2]};
                
                float finalNormal[] ={(normal1[0] + normal2[0] + normal3[0] + normal4[0])/4,(normal1[1] + normal2[1] + normal3[1] + normal4[1])/4,(normal1[2] + normal2[2] + normal3[2] + normal4[2])/4};
                
                normalize(finalNormal,finalNormal);
                
                newNormal[i][j][0] = finalNormal[0];
                newNormal[i][j][1] = finalNormal[1];
                newNormal[i][j][2] = finalNormal[2];
            }
...
int main()
{
    ...
    //cout<<normal[1][1][0]<<endl;
    
    for (int i=0;i<iter;++i)
        applySmoothing(normal,normal);
    //cout<<normal[1][1][0];
    ...
    for (int i=0; i< width; ++i)
        for (int j=0;j< height; ++j)
        {
            tempNormalized[0]=normal[i][j][0];  tempNormalized[1]=normal[i][j][1];  tempNormalized[2]=normal[i][j][2];
